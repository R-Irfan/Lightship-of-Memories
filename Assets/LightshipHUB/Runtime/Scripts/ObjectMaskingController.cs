...
namespace Niantic.ARDK.Templates 
{
    public class ObjectMaskingController : MonoBehaviour 
    {
        ...
        private void Update()
        {
            //capturedImg.transform.LookAt(aRRenderingManager.gameObject.transform);
            //capturedImg.transform.eulerAngles = new Vector3(0, capturedImg.transform.eulerAngles.y, 0);
            if (Input.GetKeyDown(KeyCode.Space))
            {
                capturedImg.GetComponent<Renderer>().material.mainTexture = aRRenderingManager.CPUTexture;
                capturedImg.GetComponent<Renderer>().material.SetTexture("_EmissionMap", aRRenderingManager.CPUTexture);
            }
        }
        ...
        IEnumerator hideMEnuButton() 
        {
            yield return new WaitForSeconds(1f);
            Debug.Log("HHIding Menu Button");
        ...
        public void showHideUi()
        {
            ...
            instructionsUIObject.SetActive(true);
            instructionsUIObject.GetComponent<HideInstructions>().StartCoroutine("HideInstructionsCoroutine");
        }
